...
#### 1- compression ####
MRIaggr.Pat1red <- constCompressMRIaggr(MRIaggr.Pat1,factor=2,trace=TRUE)
...
res <- calcTableHypoReperf(MRIaggr.Pat1,param=c("TTP","MTT"),time=c("t0"),
...
res <- calcTableHypoReperf(MRIaggr.Pat1,param=c("TTP","MTT"),time=c("t0","t1"),
                           mask="MASK_DWI_t0",
                           overwrite=TRUE,update.object=TRUE)
...
calcROCthreshold(MRIaggr.Pat1,mask=c("MASK_DWI_t0","MASK_T2_FLAIR_t2"),param=c("DWI_t0","T2_FLAIR_t2"),
                  as.logical=TRUE,plot="ROC_Youden",update.object=TRUE,overwrite=TRUE)
...
res <- calcFilter(MRIaggr.Pat1,param=c("T2_FLAIR_t2","DWI_t0","TTP_t0"),
                 filter="2D_M3",w_contrast=FALSE,na.rm=FALSE,update.object=TRUE,overwrite=TRUE)
...
res <- calcTableLesion(MRIaggr.Pat1,maskN=c("MASK_DWI_t0","MASK_T2_FLAIR_t2"),mask="mask",
                   as.logical=TRUE,update.object=FALSE,overwrite=TRUE)
