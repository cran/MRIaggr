...
resG_w.2D <- calcFilter(M,filter="2D_G3",w_contrast=TRUE)
...
resSx_w.2D <- calcFilter(M,filter="2D_Sx",w_contrast=TRUE)
resSy_w.2D <- calcFilter(M,filter="2D_Sy",w_contrast=TRUE)
...
resG_w.2D_NArm <- calcFilter(M,filter="2D_G3",w_contrast=TRUE,na.rm=TRUE)
...
resSx_w.2D_NArm <- calcFilter(M,filter="2D_Sx",w_contrast=TRUE,na.rm=TRUE)
resSy_w.2D_NArm <- calcFilter(M,filter="2D_Sy",w_contrast=TRUE,na.rm=TRUE)
