...
nifti.Pat1_MTT_t0 <- readMRI(file=file.path(path,"MTT_t0"),format="nifti")
nifti.Pat1_TTP_t0 <- readMRI(file=file.path(path,"TTP_t0"),format="nifti")

nifti.Pat1_MTT_t1 <- readMRI(file=paste(path,"MTT_t1",sep="/"),format="nifti")
nifti.Pat1_TTP_t1 <- readMRI(file=paste(path,"TTP_t1",sep="/"),format="nifti")

nifti.Pat1_DWI_t0 <- readMRI(file=paste(path,"DWI_t0",sep="/"),format="nifti")
nifti.Pat1_T1_t0 <- readMRI(file=paste(path,"T1_t0",sep="/"),format="nifti")
nifti.Pat1_T2_GRE_t0 <- readMRI(file=paste(path,"T2_GRE_t0",sep="/"),format="nifti")
nifti.Pat1_T2_FLAIR_t2 <- readMRI(file=paste(path,"T2_FLAIR_t2",sep="/"),format="nifti")

nifti.Pat1_MASK_DWI_t0 <- readMRI(file=paste(path,"MASK_DWI_t0",sep="/"),format="nifti")
nifti.Pat1_MASK_T2_FLAIR_t2 <- readMRI(file=paste(path,"MASK_T2_FLAIR_t2",sep="/"),format="nifti")
...
for(iter_file in 1:n.files){
  assign(x=paste("nifti.Pat1_",param_nifti[iter_file],sep=""),
         value=readMRI(file=paste(path,param_nifti[iter_file],sep="/"),
                       format="nifti")
  )  
}
...
ls.array <- list()
for(iter_file in 1:n.files){
  ls.array[[iter_file]] <- readMRI(file=paste(path,param_nifti[iter_file],sep="/"),
                                   format="nifti")  
}
